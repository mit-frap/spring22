...
Here's a simple [Fixpoint] definition [always_true : nat -> nat] that deserves its name:
|*)

Fixpoint always_true (x : nat) {struct x} : bool := true.
...
Theorem always_true_ok : forall x : nat,
    always_true x = true.
Proof.
...
Here's a key fact to understanding how [simplify] works on [Fixpoint] definitions: [simplify] only reduces the application of [Fixpoint] definition when the argument on which it structurally recurses (according to Coq) is the form of a constructor application.
