...
In the proof of preservation for `step0`, you will have `hasty` hypotheses for for expressions with known constructors, e.g. for an `(Abs x e)` or for a `(TupleCons e1 e2)` etc.
Without subtyping, inverting such a `hasty` would give you just one subgoal, where the `hasty` is replaced by the preconditions that were used to construct it.
